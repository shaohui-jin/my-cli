 ...
 * 5. 不存在就默认30s存储时间
 */
export const addCookie: (key: string, value: string, extra?: Extra) => boolean = (key: string, value: string, extra: Extra = { type: 'sessionStorage' }) => {
  try {
    ...
    window.document.cookie = `${key}=${value};expires=${new Date(date)};domain=${extra.domain || document.domain};path=${extra.path || '/'}`;
 ...
 * return boolean
 */
export const removeCookie: (key: string, extra?: Extra) => boolean = (key: string, extra: Extra = { type: 'sessionStorage' }) => {
  try {
    if (extra.type === 'localStorage' && !checkUndefined(localStorage)) {
      // 类型是localStorage且localStorage存在，则使用这个
    ...
    window.document.cookie = `${key}=;expires=${new Date(date)};domain=${extra.domain || document.domain};path=${extra.path || '/'}`;
  ...
  try {
    ...
    if (keys) {
      for (let i = keys.length; i > 0; i--) {
        document.cookie = `${keys[i]}=;path=${extra.path || '/'};domain=${extra.domain || document.domain};expires=${new Date(0).toUTCString()}`;
      }
    }
 ...
 * return value值
 */
export const getCookie: (key: string, extra?: Extra) => string | null = (key: string, extra: Extra = { type: 'sessionStorage' }) => {
  try {
    if (extra.type === 'localStorage' && !checkUndefined(localStorage)) {
      // 类型是localStorage且localStorage存在，则使用这个
